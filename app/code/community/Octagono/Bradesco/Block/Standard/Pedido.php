 ...
 */
class Octagono_Bradesco_Block_Standard_Pedido extends Mage_Payment_Block_Form
{
    ...
    protected function _toHtml()
    {
		...
		if (empty($order_id)) {
			...
			$html = Mage::getModel('bradesco/standard')->getStandardCheckoutFormFields($order_id);
		}
	...
	private function getFormCode() {
		...
		$parcelas = $standard->getOrder($orderId)->getQuote()->getPayment()->getData('cc_parcelas');
		...
		$transOrig = '04';
		if ($numParcelas > 1) {
			if ($standard->getConfigData('juros') == '1') { //juros do emisor / com juros
				$transOrig = '06';
			}
			else {
		...
		$total = $standard->getOrder($orderId)->getQuote()->getGrandTotal();

		$form = '<form id = "bradesco_standard_checkout" name="bradesco_standard_checkout" action="http://ecommerce.bradesco.com.br/pos_virtual/confirma.asp" method="post">';
		...
		$form .= '<input type="hidden" name="FILIACAO" value="' . $standard->getConfigData('codigo_bradesco') . '" />';
	...
	private function getValores() {
		...
		$orderId = $_REQUEST['NUMPEDIDO'];
		$order = $standard->getOrder($orderId);
		$quote = $standard->getQuote($order->getQuoteId());
		...
		$valores = $valores . "&FILIACAO=" . $standard->getConfigData('codigo_bradesco');
