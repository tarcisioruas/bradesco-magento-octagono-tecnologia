 ...
 */
class Octagono_Bradesco_Block_Transferencia_Sendmail extends Mage_Core_Block_Template
{
	...
	protected function _toHtml()
    {
		...
		$order = $transferencia->getOrder($order_id);
	...
	public function sendEmail($subject, $from, $to, $order_id, $order) {
		...
		$html .= '<a href="' . Mage::getUrl("Bradesco/transferencia/view/order_id/$order_id") . '" target="boleto">Segunda via do boleto.</a>';
		$html .= '<br /><br />';
		$html .= 'Atenciosamente, <br /><br />';
		$html .= Mage::getStoreConfig('system/store/name', $order->getStoreId());

		$ret = $this->getTransferencia()->sendHTMLemail($html, $from, $to, $subject);
