...
function getPosSeparador($endereco, $procuraEspaco = false) {
    ...
    public function preparaCampo($campo) {
    	...
    	$campo = htmlentities($campo); //codifica entidades HTML. Isso evita que o usu�rio digite algum comando HTML que interfira na p�gina
    ...
    public function captura($orderId, $tid, $responseCode, $message, $amount) {
		...
		$textoCaptura = "Valor Capturado: $amount. C�digo da Transa��o: $tid. C�digo de Resposta da Transa��o: $responseCode. Mensagem: $message.";
		...
		if ($responseCode=='0' || $responseCode=='3') {
			if (!$order->canInvoice()) {
				...
				$ret = 'Este pedido n�o pode gerar Fatura. Verifique se a fatura j� foi criada.';
    ...
    public function autoriza($orderId, $tid, $responseCode, $authCode, $message, $amount=0) {
		...
		$textoAutorizacao = "Valor: $amount. C�digo da Transa��o: $tid. C�digo de Resposta da Transa��o: $responseCode. C�digo de Autoriza��o: $authCode. Mensagem: $message.";
		...
		/*if ($amount != $order->getGrandTotal()) {
			...
			$order->addStatusToHistory(
				$order->getStatus(),//continue setting current order status
				'Total do pedido n�o confere com o valor aprovado pela Bradesco. ' . $textoAutorizacao
			);
		}*/
		...
		else { //se a transa��o n�o foi aprovada
			$order->addStatusToHistory(
				'Cancelado', //$order->getStatus(),//continue setting current order status
				'Transa��o n�o aprovada. ' . $textoAutorizacao
			);
		}
	...
	public function capture(Varien_Object $payment, $amount)
    {
		...
		if ($payment->getCcTransId()) {
			...
			Mage::throwException('C�digo da transa��o n�o identificado');
		}
		return $this;
	}*/
    ...
    /*public function capture(Varien_Object $payment, $amount)
    {
		...
		if ($payment->getCcTransId()) {
			...
			else
			{
				...
				{
					Mage::throwException('Falha na captura. Informa��o inconsistente: ' . $result['ars'] . '. LR: ' . $result['lr'] . '. Cap: ' . $result['cap']);
				}
				else if ($result['lr'] == '3' || $result['lr'] == 3)
				{
					Mage::throwException('Captura j� efetuada: ' . $result['ars'] . '. LR: ' . $result['lr'] . '. Cap: ' . $result['cap']);
				}
				else
				{
